...
public class VenueDirectory {
    ...
    public VenueDirectory(Connection connection) {
        this.connection = connection;
    }

    public ResultSet getVenueData() {
        try {
    ...
    public ResultSet getVenues() {
        try {
            PreparedStatement st = (PreparedStatement) connection.prepareStatement("select * from city_applications where type = 'venue' and application_status = 'Approved'");
            return st.executeQuery();
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    public void book_ticket(String uname, String venue, String event, int tickets, String applied_date) {
        try {
            PreparedStatement st = (PreparedStatement) connection.prepareStatement("insert into event_bookings(username,venue,event,tickets,event_date) values(?,?,?,?,?)");
    ...
    public ResultSet get_bookings(String name) {
        try {
            PreparedStatement st = (PreparedStatement) connection.prepareStatement("select * from event_bookings  where username = ?");
            st.setString(1, name);
            return st.executeQuery();
        } catch (SQLException e) {
            System.out.println(e);
